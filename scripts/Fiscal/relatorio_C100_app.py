# ===== ANALISADOR SPED FISCAL v2.3 - COM TIPO DE OPERAÇÃO (ENTRADA/SAÍDA) =====
...
def processar_bloco_nota(c100_data, temp_c170_list, temp_c190_list, mapas):
    """
    Função auxiliar que processa os itens de UMA nota fiscal, aplicando a lógica condicional.
    """
    linhas_geradas = []
    mapa_produtos = mapas.get('produtos', {})
    mapa_cfop = mapas.get('cfop', {})

    if temp_c170_list:
        # Prioridade 1: Processa os registros C170 se existirem
        for campos in temp_c170_list:
            ...
            row = {
                **c100_data,
                ...
                'Vlr Base Cálculo ICMS': safe_float_convert(safe_get(campos, 12)),
                'Alíquota ICMS (%)': safe_float_convert(safe_get(campos, 13)),
                'Vlr ICMS': safe_float_convert(safe_get(campos, 14)),
            ...
            row['Descrição CFOP'] = mapa_cfop.get(row['CFOP'], '')
            linhas_geradas.append(row)
    elif temp_c190_list:
        # Prioridade 2: Processa os C190 SOMENTE SE não houver C170
        for campos in temp_c190_list:
            ...
            row = {
                **c100_data,
                ...
                'CFOP': cfop,
                'Descrição CFOP': mapa_cfop.get(cfop, ''),
...
def gerar_relatorio_bloco_c(lista_arquivos):
    """Função principal que implementa a lógica de extração para o Bloco C."""
    
    mapa_empresa, mapa_participantes, mapa_produtos, mapa_cfop = {}, {}, {}, {}
        ...
        with open(filepath, 'r', encoding='latin-1') as f:
            for line in f:
                ...
                if reg == '0000' and nome_arquivo not in mapa_empresa:
                    mapa_empresa[nome_arquivo] = {'CNPJ': safe_get(campos, 5), 'Inscrição Estadual': safe_get(campos, 7), 'Período': f"{format_date(safe_get(campos, 3))} a {format_date(safe_get(campos, 4))}"}
                elif reg == '0150':
                    ...
                    if cod_item and cod_item not in mapa_produtos:
                        mapa_produtos[cod_item] = {'Descrição Item': safe_get(campos, 2), 'NCM': safe_get(campos, 7)}
                elif reg == '0400':
                    cod_nat = safe_get(campos, 1)
                    if cod_nat and cod_nat not in mapa_cfop:
                        mapa_cfop[cod_nat] = safe_get(campos, 2)
    
    mapas_gerais = {'produtos': mapa_produtos, 'cfop': mapa_cfop}
    report_rows = []
    ...
    for filepath in lista_arquivos:
        ...
        with open(filepath, 'r', encoding='latin-1') as f:
            ...
            for line in f:
                ...
                if (reg.startswith('C100') or not reg.startswith('C1')) and current_c100_data:
                    report_rows.extend(processar_bloco_nota(current_c100_data, temp_c170_list, temp_c190_list, mapas_gerais))
                ...
                if reg == 'C100':
                    ...
                    current_c100_data = {
                        **info_empresa,
                        ...
                        'Data Documento': format_date(safe_get(campos, 9)),
                        'Vlr Documento': safe_float_convert(safe_get(campos, 11))
                    }
    ...
    # --- COLUNA ADICIONADA AO LAYOUT FINAL ---
    colunas_finais = ['CNPJ', 'Inscrição Estadual', 'Período', 'Data Documento', 'Tipo Operação', 'Número Documento', 'Chave NF-e', 'Código Participante', 'Nome Participante', 'Vlr Documento', 'Origem', 'Código Item', 'Descrição Item', 'NCM', 'Qtde Item', 'Vlr Item', 'CFOP', 'Descrição CFOP', 'CST ICMS', 'Vlr Operação', 'Vlr Base Cálculo ICMS', 'Alíquota ICMS (%)', 'Vlr ICMS', 'CST PIS', 'Vlr Base Cálculo PIS', 'Alíquota PIS (%)', 'Vlr PIS', 'CST COFINS', 'Vlr Base Cálculo COFINS', 'Alíquota COFINS (%)', 'Vlr COFINS', 'Conta Contábil']
...
class SpedReportApp(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle("Gerador de Relatório SPED Fiscal - Bloco C")
    ...
    def start_processing(self):
        ...
        try:
            df_result = gerar_relatorio_bloco_c(self.selected_files)
            self.save_report(df_result)
            self.status_label.setText("✅ Relatório consolidado do Bloco C gerado com sucesso!")
    ...
    def save_report(self, dataframe):
        default_filename = f"Relatorio_Consolidado_Bloco_C_{datetime.now().strftime('%Y%m%d')}.xlsx"
